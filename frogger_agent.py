...
from state_space import capture_state
...
pygame.display.set_caption('Frogger Agent')
...
while True:
    ...
    while frog.lives > 0:
        ...
        # AGENT DECISION ONLY PART OF THIS FILE THAT IS OUR CODE
        if frog.can_move == 1:
            state = capture_state(frog, enemys, plataforms, chegaram, game, ticks_enemys, ticks_plataforms, ticks_time)
            key_pressed = mcts_decision(state)
            # UP TO HERE
            
            frog.moveFrog(key_pressed, key_up=1)
            frog.cannotMove()
        if not ticks_time:
            ticks_time = 30
            game.decTime()
        ...
        text_info3 = info_font.render(('Action: {0}'.format(key_pressed)),1,(255,255,0))
        screen.blit(background, (0, 0))
        ...
        frog.animateFrog(key_pressed, key_up=1)
        frog.draw()
